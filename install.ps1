...
$fontInstalled = $false
foreach ($flag in @("--user", "")) {
    if ($fontInstalled) { break }
    try {
        if ($flag) { $output = oh-my-posh font install CascadiaCode $flag 2>&1 | Out-String }
        else { $output = oh-my-posh font install CascadiaCode 2>&1 | Out-String }
